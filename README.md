...
My motivation for creating this project was firstly to aid in my understanding of pathfinding algorithms. Though I was able to grasp it conceptually, I also like to see how concepts can be applied practically. Given their popularity in route planning functionality within navigation apps, I decided a pathfinding feature within a maze drawing app would be a suitable application. This program uses _Dijkstra's algorithm_ to solve the maze. Future iterations will include _A\*_ and _Greedy Breadth First Search_.
...
This app has been deployed on Netlify, you can view it [here](https://maze-pathfinder.netlify.app/).

If you would like to run this locally, then ensure you have Node.js installed before cloning this repo.

After you have navigated into the directory, you can install the dependencies by running:
...
> Users are initially presented with an empty grid with prepositioned start (green) and finish (red) nodes
> They can click on an empty (white) node to turn that node into a wall
...
> If a User wishes to remove a particular wall node, they can click on it and it will be reverted back to an empty node
> If they wish to change the location of the start node, they can click on it and hold and drag their cursor to its desired new position
...
I would like to see a real time animation of the grid nodes that the algorithm has searched

As a User
So that I can see the solution to the maze
I would like to see an animation of the shortest path taken from the start node to finish node
```

> When the User clicks the "Run Algorithm" button, the "visited" nodes that the algorithm has searched through are highlighted
> Nodes that the algorithm has just visited are coloured dark blue
> Nodes that the algorithm has visited at an earlier stage are coloured green and then eventually light blue
...
> On completion of the animation, the User is able to clear the screen with a single button click, as opposed to individually erasing each wall node
