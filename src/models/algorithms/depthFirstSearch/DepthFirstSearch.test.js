...
describe("depth first search", () => {
  describe("for a solvable maze", () => {
    ...
    const dfs = new DepthFirstSearch(grid, grid[3][2], grid[3][4]);
    const visitedNodesInOrder = dfs.getVisitedNodes();

    it("returns an array of visited nodes in search order", () => {
      for (let i = 0; i < visitedNodesInOrder.length; i++) {
        expect(visitedNodesInOrder[i].row).toEqual(expectedDFSResult[i].row);
        expect(visitedNodesInOrder[i].col).toEqual(expectedDFSResult[i].col);
      }
    });

    it("returns an array of the shortest path nodes in order", () => {
      const finishNode = visitedNodesInOrder.slice(-1)[0];
      const shortestPathNodesInOrder = dfs.getShortestPath(finishNode);
  ...
  describe("for an unsolvable maze", () => {
    ...
    const dfs = new DepthFirstSearch(grid, grid[0][0], grid[3][4]);
    const visitedNodesInOrder = dfs.getVisitedNodes();
    it("returns false", () => {
